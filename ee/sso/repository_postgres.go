...
func NewPostgresSSORepository(engine *database.Engine) *PostgresSSORepository {
	return &PostgresSSORepository{engine: engine}
}

func (s *PostgresSSORepository) GetConfig(ctx context.Context) (*SSOConfig, error) {
...
func (s *PostgresSSORepository) SaveConfig(ctx context.Context, cfg SSOConfig) error {
...
func (s *PostgresSSORepository) DeleteConfig(ctx context.Context) error {
...
func (s *PostgresSSORepository) FindUserBySubject(ctx context.Context, issuer string, subject string) (repository.User, error) {
	...
	if err != nil {
		...
		return repository.User{}, fmt.Errorf("failed to resolve the sso identity from database: %w", err)
	}
	return userFromPgdbRow(row), nil
}

func (s *PostgresSSORepository) LinkIdentity(ctx context.Context, issuer string, subject string, userID string, email string) error {
...
func (s *PostgresSSORepository) ProvisionUser(ctx context.Context, params repository.InsertUserParameters, issuer string, subject string) (repository.User, error) {
	...
	err := s.engine.WithTx(ctx, func(q *pgdb.Queries) error {
		row, err := q.InsertUser(ctx, pgdb.InsertUserParams{
			ID:           repository.ToPgUUID(params.ID),
			Email:        email,
		...
		if err != nil {
			if database.IsUniqueViolation(err) {
				return &repository.ErrResourceAlreadyExists{Resource: "user", Identifier: email}
			}
...
func (s *PostgresSSORepository) TouchLastLogin(ctx context.Context, issuer string, subject string) error {
...
func userFromPgdbRow(row pgdb.User) repository.User {
	user := repository.User{
		Id:           row.ID.String(),
		Email:        row.Email,
