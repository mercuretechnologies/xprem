...
func setupSSOStore(t *testing.T) (*PostgresSSORepository, *pgxpool.Pool) {
	t.Helper()
	...
	return NewPostgresSSORepository(&database.Engine{Queries: pgdb.New(pool), DB: pool}), pool
}
...
func TestSSOIdentityLifecycle(t *testing.T) {
	...
	provisioned, err := ssoStore.ProvisionUser(ctx, repository.InsertUserParameters{
	...
	_, err = ssoStore.ProvisionUser(ctx, repository.InsertUserParameters{
