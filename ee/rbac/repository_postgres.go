...
func NewPostgresRBACRepository(engine *database.Engine) *PostgresRBACRepository {
	return &PostgresRBACRepository{engine: engine}
}
...
func (s *PostgresRBACRepository) ListRoles(ctx context.Context) ([]Role, error) {
...
func (s *PostgresRBACRepository) GetRoleByID(ctx context.Context, id string) (Role, error) {
	row, err := s.engine.Queries.GetRoleByID(ctx, repository.ToPgUUID(id))
	if err != nil {
...
func (s *PostgresRBACRepository) InsertRole(ctx context.Context, role Role) (Role, error) {
	row, err := s.engine.Queries.InsertRole(ctx, pgdb.InsertRoleParams{
		ID:          repository.ToPgUUID(role.ID),
		Name:        role.Name,
		Permissions: fromPermissions(role.Permissions),
	})
	if err != nil {
		if database.IsUniqueViolation(err) {
			return Role{}, &repository.ErrResourceAlreadyExists{Resource: "role", Identifier: role.Name}
		}
...
func (s *PostgresRBACRepository) UpdateRole(ctx context.Context, id string, name string, permissions []Permission) error {
	commandTag, err := s.engine.Queries.UpdateRole(ctx, pgdb.UpdateRoleParams{
		ID:          repository.ToPgUUID(id),
		Name:        name,
		Permissions: fromPermissions(permissions),
	})
	if err != nil {
		if database.IsUniqueViolation(err) {
			return &repository.ErrResourceAlreadyExists{Resource: "role", Identifier: name}
		}
...
func (s *PostgresRBACRepository) DeleteRole(ctx context.Context, id string) error {
	commandTag, err := s.engine.Queries.DeleteRole(ctx, repository.ToPgUUID(id))
	if err != nil {
...
func (s *PostgresRBACRepository) ListUserGrants(ctx context.Context, userID string) ([]AppGrant, error) {
	rows, err := s.engine.Queries.ListUserAppGrants(ctx, repository.ToPgUUID(userID))
	if err != nil {
...
func (s *PostgresRBACRepository) GetUserAppGrant(ctx context.Context, userID string, appID string) (*AppGrant, error) {
...
func (s *PostgresRBACRepository) ReplaceUserGrants(ctx context.Context, userID string, grants []GrantInput) error {
	pgUserID := repository.ToPgUUID(userID)
	err := s.engine.WithTx(ctx, func(q *pgdb.Queries) error {
		...
		for _, grant := range grants {
			...
			if grant.RoleID != nil {
				roleID = repository.ToPgUUID(*grant.RoleID)
				if !roleID.Valid {
			...
			if err := q.InsertUserAppGrant(ctx, pgdb.InsertUserAppGrantParams{
				UserID:           pgUserID,
				AppID:            repository.ToPgUUID(grant.AppID),
				RoleID:           roleID,
...
func (s *PostgresRBACRepository) GrantCountsByUser(ctx context.Context) (map[string]int64, error) {
...
func (s *PostgresRBACRepository) ListAccessibleAppIDs(ctx context.Context, userID string) ([]string, error) {
	rows, err := s.engine.Queries.ListAccessibleAppIDs(ctx, repository.ToPgUUID(userID))
	if err != nil {
