...
func NewPostgresAuditRepository(engine *database.Engine) *PostgresAuditRepository {
	return &PostgresAuditRepository{engine: engine}
}
...
func (s *PostgresAuditRepository) Insert(ctx context.Context, event Event) (Event, error) {
	var appID *string
	if event.AppID != "" {
		appID = &event.AppID
	}
...
func (s *PostgresAuditRepository) List(ctx context.Context, params ListParams) ([]Event, error) {
	rows, err := s.engine.Queries.ListAuditLogEvents(ctx, pgdb.ListAuditLogEventsParams{
		ActorID:      params.ActorID,
		Action:       params.Action,
		AppID:        params.AppID,
		Outcome:      params.Outcome,
		OccurredFrom: repository.ToPgTimestamptz(params.From),
		OccurredTo:   repository.ToPgTimestamptz(params.To),
...
func (s *PostgresAuditRepository) PurgeBefore(ctx context.Context, cutoff time.Time, exportedOnly bool) (int64, error) {
...
func (s *PostgresAuditRepository) ListAfter(ctx context.Context, afterID int64, limit int) ([]Event, error) {
...
func (s *PostgresAuditRepository) ExportCursor(ctx context.Context) (int64, error) {
...
func (s *PostgresAuditRepository) TryExportLock(ctx context.Context) (func(), bool, error) {
	return postgres.TryAdvisoryLock(ctx, s.engine.DB, postgres.AuditExportLockID, "audit export")
}

func (s *PostgresAuditRepository) AdvanceExportCursor(ctx context.Context, from int64, to int64) (bool, error) {
...
func (s *PostgresAuditRepository) Count(ctx context.Context, filters ListFilters) (int64, error) {
	count, err := s.engine.Queries.CountAuditLogEvents(ctx, pgdb.CountAuditLogEventsParams{
		ActorID:      filters.ActorID,
		Action:       filters.Action,
		AppID:        filters.AppID,
		Outcome:      filters.Outcome,
		OccurredFrom: repository.ToPgTimestamptz(filters.From),
		OccurredTo:   repository.ToPgTimestamptz(filters.To),
	})
	if err != nil {
