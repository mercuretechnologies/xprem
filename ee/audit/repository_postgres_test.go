...
func setupAuditStore(t *testing.T) (*PostgresAuditRepository, *pgxpool.Pool) {
	t.Helper()
	...
	return NewPostgresAuditRepository(&database.Engine{Queries: pgdb.New(pool), DB: pool}), pool
}

func insertTestEvent(t *testing.T, auditStore *PostgresAuditRepository, event Event) Event {
	t.Helper()
