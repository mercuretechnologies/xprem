...
func NewPostgresIdentityRepository(engine *database.Engine) *PostgresIdentityRepository {
	return &PostgresIdentityRepository{engine: engine}
}
...
func (s *PostgresIdentityRepository) GetSchema(ctx context.Context, appID string) (Schema, error) {
...
func (s *PostgresIdentityRepository) UpsertSchemaKey(ctx context.Context, appID string, spec KeySpec) (KeySpec, error) {
	if spec.MaxLength == 0 {
...
func (s *PostgresIdentityRepository) DeleteSchemaKey(ctx context.Context, appID string, key string) (bool, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return false, err
	}
...
func (s *PostgresIdentityRepository) ApplySet(ctx context.Context, appID string, easClientID string, raw map[string]any, geo *Geo) (ApplyResult, error) {
...
func (s *PostgresIdentityRepository) ApplySetOnce(ctx context.Context, appID string, easClientID string, raw map[string]any, geo *Geo) (ApplyResult, error) {
...
func (s *PostgresIdentityRepository) ApplyUnset(ctx context.Context, appID string, easClientID string, keys []string, geo *Geo) (ApplyResult, error) {
...
func (s *PostgresIdentityRepository) mutate(ctx context.Context, appID string, easClientID string, kind mutationKind, raw map[string]any, unsetKeys []string, geo *Geo) (ApplyResult, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return ApplyResult{}, err
	}
	clientUUID, err := repository.ParsePgUUID(easClientID)
	if err != nil {
		return ApplyResult{}, err
	}
...
func (s *PostgresIdentityRepository) GetDevice(ctx context.Context, appID string, easClientID string) (*Device, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return nil, err
	}
...
func deviceFilterParams(query DeviceQuery) (convertedDeviceFilters, error) {
	...
	updateIDs, err := repository.ParsePgUUIDs(query.CurrentUpdateIDs)
	if err != nil {
		return convertedDeviceFilters{}, err
	}
...
func (s *PostgresIdentityRepository) ListDevices(ctx context.Context, appID string, query DeviceQuery, limit int, cursor *DeviceCursor) ([]Device, *DeviceCursor, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return nil, nil, err
	}
	...
	if cursor != nil {
		...
		cursorUUID, err := repository.ParsePgUUID(cursor.EASClientID)
		if err != nil {
			return nil, nil, err
		}
...
func (s *PostgresIdentityRepository) CountOnlineDevices(ctx context.Context, appID string, since time.Time, query DeviceQuery) (int64, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return 0, err
	}
...
func (s *PostgresIdentityRepository) SearchMetadataValues(ctx context.Context, appID string, key string, search string, limit int) ([]ValueCount, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return nil, err
	}
...
func (s *PostgresIdentityRepository) TouchDevice(ctx context.Context, appID string, easClientID string, geo *Geo, current *CurrentUpdate, device DeviceInfo) error {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return err
	}
	...
	if current != nil {
		if currentUpdate, err = repository.ParsePgUUID(current.ID); err != nil {
			return err
		}
...
func (s *PostgresIdentityRepository) resolveUpdateFailures(
	ctx context.Context,
...
func (s *PostgresIdentityRepository) RecordUpdateFailures(ctx context.Context, appID string, easClientID string, updateIDs []string, fatalError string, failureType FailureType) error {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return err
	}
	...
	for _, updateID := range updateIDs {
		updateUUID, err := repository.ParsePgUUID(updateID)
		if err != nil {
...
func (s *PostgresIdentityRepository) RecordRuntimeFailure(ctx context.Context, appID string, easClientID string, updateID string, fatalError string, occurredAt time.Time) error {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return err
	}
...
func (s *PostgresIdentityRepository) ResolveRuntimeFailure(ctx context.Context, appID string, easClientID string, updateID string, occurredAt time.Time) error {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return err
	}
...
func (s *PostgresIdentityRepository) UpdateHealthByIDs(ctx context.Context, appID string, updateIDs []string) (map[string]UpdateHealth, error) {
	appUUID, err := repository.ParsePgUUID(appID)
	if err != nil {
		return nil, err
	}
	...
	for _, raw := range updateIDs {
		if parsed, err := repository.ParsePgUUID(raw); err == nil {
	...
	for _, raw := range updateIDs {
		if _, err := repository.ParsePgUUID(raw); err == nil {
