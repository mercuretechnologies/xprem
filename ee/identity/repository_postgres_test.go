...
import (
	...
	pgrepository "xprem/internal/repository"
...
func setupIdentityStore(t *testing.T) (*PostgresIdentityRepository, *pgxpool.Pool) {
	t.Helper()
	...
	store := NewPostgresIdentityRepository(&database.Engine{Queries: pgdb.New(pool), DB: pool})
	return store, pool
}
...
func declareKey(t *testing.T, store *PostgresIdentityRepository, appID, key string, valueType ValueType) {
	t.Helper()
...
func TestUpdateHealthCounts(t *testing.T) {
	...
	appUUID, err := pgrepository.ParsePgUUID(appID)
	require.NoError(t, err)
	updateAUUID, err := pgrepository.ParsePgUUID(updateA)
	require.NoError(t, err)
	updateBUUID, err := pgrepository.ParsePgUUID(updateB)
	require.NoError(t, err)
