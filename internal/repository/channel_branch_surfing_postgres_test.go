...
func setupChannelStore(t *testing.T) (*repository.PostgresChannelRepository, *pgxpool.Pool) {
	t.Helper()
	_, pool := setupBranchStore(t)
	return repository.NewPostgresChannelRepository(&database.Engine{Queries: pgdb.New(pool), DB: pool}), pool
}
