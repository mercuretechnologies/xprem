...
type rolloutFixture struct {
	pool            *pgxpool.Pool
	rollouts        *repository.PostgresRolloutRepository
	updates         *repository.PostgresUpdateRepository
	branches        *repository.PostgresBranchRepository
	channels        *repository.PostgresChannelRepository
	appId           string
...
func newRolloutFixture(t *testing.T) *rolloutFixture {
	...
	fixture := &rolloutFixture{
		pool:     pool,
		rollouts: repository.NewPostgresRolloutRepository(engine),
		updates:  repository.NewPostgresUpdateRepository(engine),
		branches: repository.NewPostgresBranchRepository(engine),
		channels: repository.NewPostgresChannelRepository(engine),
...
func TestMarkUpdateAsCheckedConditionalStampPostgres(t *testing.T) {
	...
	require.ErrorIs(t, err, repository.ErrPublishBlockedByActiveRollout)
	...
	require.ErrorIs(t, err, repository.ErrRolloutSupersededByNewerUpdate)
