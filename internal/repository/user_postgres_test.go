...
func setupUserStore(t *testing.T) (*repository.PostgresUserRepository, *pgxpool.Pool) {
	t.Helper()
	...
	return repository.NewPostgresUserRepository(&database.Engine{Queries: pgdb.New(pool), DB: pool}), pool
}
...
func insertUser(t *testing.T, userStore *repository.PostgresUserRepository, email string, isAdmin bool) repository.User {
	t.Helper()
	user, err := userStore.InsertUser(context.Background(), repository.InsertUserParameters{
		ID:           uuid.NewString(),
		Email:        email,
...
func TestGuardedQueriesRefuseRemovingLastAdmin(t *testing.T) {
	...
	require.ErrorIs(t, userStore.UpdateUserIsAdmin(ctx, admin.Id, false), repository.ErrWouldLeaveNoAdmin)
	require.ErrorIs(t, userStore.DeleteUserByID(ctx, admin.Id), repository.ErrWouldLeaveNoAdmin)
			...
			for i := 0; i < 20; i++ {
				...
				for _, opErr := range results {
					if opErr == nil {
						...
						require.True(t, errors.Is(opErr, repository.ErrWouldLeaveNoAdmin),
