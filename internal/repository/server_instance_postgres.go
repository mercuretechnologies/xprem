...
// PostgresServerInstanceRepository is the server_instance singleton: the
// deployment's permanent UUID, minted on first boot and immutable after.
type PostgresServerInstanceRepository struct {
	engine *database.Engine
}

func NewPostgresServerInstanceRepository(engine *database.Engine) *PostgresServerInstanceRepository {
	return &PostgresServerInstanceRepository{engine: engine}
}
...
func (s *PostgresServerInstanceRepository) GetOrCreateInstanceID(ctx context.Context, seed string) (string, error) {
