...
func setupRefreshTokenStore(t *testing.T) (*repository.PostgresRefreshTokenRepository, *repository.PostgresUserRepository, *pgxpool.Pool) {
	t.Helper()
	...
	return repository.NewPostgresRefreshTokenRepository(engine), repository.NewPostgresUserRepository(engine), pool
}

func insertRefreshToken(t *testing.T, tokenStore *repository.PostgresRefreshTokenRepository, userId string, familyId string, expiresAt time.Time) string {
	t.Helper()
	id := uuid.NewString()
	require.NoError(t, tokenStore.InsertRefreshToken(context.Background(), repository.InsertRefreshTokenParameters{
		ID:        id,
		UserID:    userId,
		FamilyID:  familyId,
		ExpiresAt: expiresAt,
...
func TestRefreshTokenLedgerLifecycle(t *testing.T) {
	...
	consumed, err := tokenStore.RotateRefreshToken(ctx, repository.RotateRefreshTokenParameters{
		OldID:     tokenId,
		NewID:     successorId,
	...
	_, err = tokenStore.RotateRefreshToken(ctx, repository.RotateRefreshTokenParameters{
	...
	for range 20 {
		...
		for i := range results {
			...
			go func() {
				defer done.Done()
				start.Wait()
				_, results[i] = tokenStore.RotateRefreshToken(ctx, repository.RotateRefreshTokenParameters{
					OldID: tokenId, NewID: uuid.NewString(), ExpiresAt: time.Now().Add(time.Hour),
				})
			}()
		}
