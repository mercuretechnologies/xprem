...
func setupBranchStore(t *testing.T) (*repository.PostgresBranchRepository, *pgxpool.Pool) {
	t.Helper()
	...
	return repository.NewPostgresBranchRepository(&database.Engine{Queries: pgdb.New(pool), DB: pool}), pool
}
...
func TestDeleteBranchRefusesProtectedBranch(t *testing.T) {
	...
	protectedErr := (*repository.ErrBranchProtected)(nil)
